...
All three pages are generated by `build_pages.py` (a copy of the generator from the GLO Cowork session) so the
header, footer, calculators, FAQs and shared sections stay identical. The script depends on files that only
exist in that session, so treat it as documentation of how the pages fit together: if you edit one page by
hand, make the same change in the other two, or ask Claude to regenerate them.
